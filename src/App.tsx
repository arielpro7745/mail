...
// === לו"ז 12 ימים (סבב 1) ===
// כל הרחובות נכנסים בסבב אחד של 12 ימי עבודה.
// אזור 14 מרוכז ביום אחד (כולל רוטשילד זוגי + אי-זוגי + קק"ל) כי אין בו הרבה דואר.
const SCHEDULE_12_DAYS = [
  // --- יום 1: אזור 7 (כחול) ---
  { day: 1, area: 7, title: "7 - פינסקר 1 עד 67", streets: ["פינסקר 1 עד 67"], subType: "7_S1", tips: "פינסקר (הכי גדול): 1–67.", bldgCount: 42 },

  // --- יום 2: אזור 14 (ירוק) ---
  { day: 2, area: 14, title: "14 - רוטשילד (זוגי+אי-זוגי), קק\"ל, הדף היומי, גד מכנס", streets: ["רוטשילד זוגי", "רוטשילד אי-זוגי", "קק\"ל", "הדף היומי", "גד מכנס 4"], subType: "14_S1", tips: "אזור 14 קטן: עושים את כל הרחובות בסבב 1 ביום אחד.", bldgCount: 25 },

  // --- יום 3: אזור 12 (אדום) ---
  { day: 3, area: 12, title: "12 - חיים כהן, שבדיה, פנקס", streets: ["חיים כהן", "שבדיה", "פנקס"], subType: "12_S1", tips: "אזור 12 - חבילה 1/4.", bldgCount: 45 },

  // --- יום 4: אזור 7 (כחול) ---
  { day: 4, area: 7, title: "7 - פינסקר 2 עד 38", streets: ["פינסקר 2 עד 38"], subType: "7_S1", tips: "פינסקר (הכי גדול): 2–38.", bldgCount: 40 },

  // --- יום 5: אזור 7 (כחול) ---
  { day: 5, area: 7, title: "7 - משה מרקוס, מקס ברוד", streets: ["משה מרקוס", "מקס ברוד"], subType: "7_S1", tips: "אזור 7 - חבילה 3/7.", bldgCount: 28 },

  // --- יום 6: אזור 12 (אדום) ---
  { day: 6, area: 12, title: "12 - הכרם, התשעים ושלוש, הרב קוק", streets: ["הכרם", "התשעים ושלוש", "הרב קוק"], subType: "12_S1", tips: "אזור 12 - חבילה 2/4.", bldgCount: 38 },

  // --- יום 7: אזור 7 (כחול) ---
  { day: 7, area: 7, title: "7 - תל חי, ברוידה", streets: ["תל חי", "ברוידה"], subType: "7_S1", tips: "אזור 7 - חבילה 4/7.", bldgCount: 26 },

  // --- יום 8: אזור 7 (כחול) ---
  { day: 8, area: 7, title: "7 - חכם יוסף חיים, האחים רוזוב", streets: ["חכם יוסף חיים", "האחים רוזוב"], subType: "7_S1", tips: "אזור 7 - חבילה 5/7.", bldgCount: 26 },

  // --- יום 9: אזור 12 (אדום) ---
  { day: 9, area: 12, title: "12 - אנה פרנק, האחים ראב, חפץ מרדכי", streets: ["אנה פרנק", "האחים ראב", "חפץ מרדכי"], subType: "12_S1", tips: "אזור 12 - חבילה 3/4.", bldgCount: 30 },

  // --- יום 10: אזור 7 (כחול) ---
  { day: 10, area: 7, title: "7 - עולי בבל, האחים שטרייט", streets: ["עולי בבל", "האחים שטרייט"], subType: "7_S1", tips: "אזור 7 - חבילה 6/7.", bldgCount: 24 },

  // --- יום 11: אזור 7 (כחול) ---
  { day: 11, area: 7, title: "7 - ליברמן, אורלוב", streets: ["ליברמן", "אורלוב"], subType: "7_S1", tips: "אזור 7 - חבילה 7/7.", bldgCount: 22 },

  // --- יום 12: אזור 12 (אדום) ---
  { day: 12, area: 12, title: "12 - מנדלסון, רוטשילד 100, זכרון משה", streets: ["מנדלסון", "רוטשילד 100", "זכרון משה"], subType: "12_S1", tips: "אזור 12 - חבילה 4/4.", bldgCount: 26 }
];
...
export default function App() {
  ...
  const streetsToShow = useMemo(() => {
    ...
    const filtered = pendingToday.filter(street => {
       ...
       return currentDaySchedule.streets.some(def => {
           ...
           if (def.includes("רוטשילד")) {
                ...
                if (def.includes("זוגי") && !def.includes("אי")) return num > 0 && num % 2 === 0 && num !== 100;
                if (def.includes("אי-זוגי")) return num > 0 && num % 2 !== 0;
                return true; 
           }
           // 3. טיפול בפינסקר (תומך בטווחים: "פינסקר 1 עד 67" / "פינסקר 2–38")
           if (def.includes("פינסקר")) {
                if (!street.name.includes("פינסקר")) return false;

                // אם הוגדר טווח (1 עד 67 / 2-38 וכו') – מסננים לפי הטווח
                const rangeMatch = def.match(/(\d+)\s*(?:-|–|עד)\s*(\d+)/);
                if (rangeMatch) {
                    const start = parseInt(rangeMatch[1], 10);
                    const end = parseInt(rangeMatch[2], 10);
                    if (!(num > 0 && num >= start && num <= end)) return false;

                    // אם זה צד אחד (תחילה/סוף באותה זוגיות) – שומרים גם על זוגי/אי-זוגי
                    if ((start % 2) === (end % 2)) return (num % 2) === (start % 2);
                    return true;
                }

                // fallback למקרים ישנים (זוגי/אי-זוגי בלי טווח)
    ...
    filtered.forEach(s => {
        let key = s.id;
        if (s.name.includes("הדף היומי")) key = "daf";
        if (s.name.includes("גד מכנס")) key = "gad";
        uniqueMap.set(key, s);
    });

    // מיון לפי הסדר ברשימה
